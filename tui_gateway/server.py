...
def _submit_prompt_to_compute_host(rid: str, sid: str, session: dict, text: Any) -> dict:
    # Isolated turns bypass _run_prompt_submit, so claim (or reuse) the
    # cross-process max_concurrent_sessions slot here, before any compute-host
    # model work — mirrors the inline claim at the top of _run_prompt_submit's
    # run(). At cap the turn never dispatches: return the session to idle and
    # surface the standard limit message as this turn's error event (same
    # shape as the inline path); a prompt queued meanwhile still drains.
    # Fail-open (lease None, no message) proceeds uncounted, like every
    # other surface.
