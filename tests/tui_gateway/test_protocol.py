...
def test_compute_host_completion_schedules_only_after_queue_drains(server, monkeypatch):
    session = _lease_test_session(agent=None)
    session["running"] = True
    scheduled = []
    ...
    server._on_compute_host_turn_done("rid", "ui-compute", session, {"type": "turn.end"})
    assert scheduled == []

    session["running"] = True
    monkeypatch.setattr(server, "_drain_queued_prompt", lambda *_args: False)
    server._on_compute_host_turn_done("rid", "ui-compute", session, {"type": "turn.end"})
    assert scheduled == [("ui-compute", session)]
